...
Homicide rate is a continuous variable and is plotted by `ggplot` as such. If we wanted to plot our map as a 'true' choropleth map we need to convert our continous variable into a categoriacal one, according to whichever brackets we want to use. 
...
```{r read-dsm, results='show', eval=FALSE}
HARV_DTM <- rast("data/HARV_dtmCrop.tif")
```

Before using ggplot we need to convert this to a dataframe. The `terra` package has an built-in function for conversion to a plotable dataframe.
