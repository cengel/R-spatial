...
`ggmap` builds on `ggplot` and allows to pull in tiled basemaps from different services, like Google Maps, OpenStreetMap, or Stamen Maps[^19].
...
- `tm_shape()` where we provide 
    - the `sf` object (we could also provide a `SpatialPolygonsDataframe`)
