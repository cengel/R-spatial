...
[^11]: Lovelace, R., Nowosad, J., & Muenchow, J. (2025). Geocomputation with R. CRC Press.
...
```{r load-edu-csv}
edu <- read_csv("data/PhiladelphiaEduAttain.csv")
names(edu)
```
...
edu_sf <- left_join(philly_sf, edu, by = c("GEOID10" = "GEOID"))
names(edu_sf) 
```
...
> in edu_sf, add a new variable  and calculate the ratio of the female population with a high school education out of the total population. (Hint: you can use mutate)
> 
> 2. Given what you have learned so far about plotting attributes create a simple
...
# One possible answer:
edu |> 
  mutate(fem_hs_ratio = fem_highschool / totalPop) |> 
  select(fem_hs_ratio) |> 
  plot()
```
...
These coordinates are already in the _USA Contiguous Albers Equal Area Conic_ projected CRS, which is the same as CRS as `philly_sf`. We can use `str_crs` to extract the projection definition from `philly_sf`, so we don't have to type it out and then directly assign it to our new centroid object.
...
philly_ctr <- 
  ...
  st_sfc(crs = st_crs(philly_sf))  # create feature collection, setting CRS to philly_sf's CRS

```


For the spatial operations we can take advantage of the suite of geometric operations that come with the `sf` package.

Let's check again the metric units of our CRS:

```{r ctr-crs, results='show'}
st_crs(philly_ctr)$proj4string
```
...
homicides_sf_aea <- st_transform(homicides_sf, st_crs(philly_sf))
```
...
range(st_coordinates(homicides_sf))
range(st_coordinates(homicides_sf_aea))
```
...
st_write(homicides_sf_aea, "data/PhillyHomicides_aea", driver = "ESRI Shapefile")
```
...
st_write(crimes_sf, "data/PhillyCrimerate", driver = "ESRI Shapefile")
```
...
# which of the CHM is less than tree height (or really low), for example
no_trees <- HARV_CHM < .5
plot(no_trees)
