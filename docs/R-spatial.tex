...
\item
  Make sure your working directory is set to \texttt{R-spatial} and contains a folder called \texttt{data}.\\
\item
  Copy, paste, and run the following lines in R:
\end{enumerate}
...
\item
  ...
  \item
    \href{https://cran.r-project.org/package=leaflet}{\texttt{leaflet}} (optional)
  \end{itemize}
\end{itemize}
...
\begin{figure}
\includegraphics[width=1\linewidth]{img/spsf_downloads} \caption{Cumulative downloads of sf and sp from CRAN}\label{fig:spsf-dl}
\end{figure}
...
#> Dimension:     XY
#> Bounding box:  xmin: 0.1501735 ymin: 0.1093448 xmax: 0.6776345 ymax: 0.9639884
#> CRS:           NA
\end{verbatim}

\begin{verbatim}
#> LINESTRING (0.1501735 0.8985337, 0.2639807 0.43...
\end{verbatim}

\begin{verbatim}
#> LINESTRING (0.6776345 0.1985623, 0.4561752 0.10...
\end{verbatim}
...
#> 1 hwy1            78 LINESTRING (0.1501735 0.898...
#> 2 hwy2            22 LINESTRING (0.6776345 0.198...
\end{verbatim}
...
\section{Creating a spatial object from a table with coordinates}\label{creating-a-spatial-object-from-a-table-with-coordinates}
...
An \texttt{sf} object can be created from a data frame in the following way. We take advantage of the \texttt{st\_as\_sf()} function which converts foreign objects into an \texttt{sf} object. Similarly to above, it requires an argument \texttt{coords}, which in the case of point data needs to be a vector that specifies the data frame's columns for the longitude and latitude (x,y) coordinates.
...
We use \texttt{read\_csv()} to read \texttt{philly\_homicides.csv} into a tibble in R and name it \texttt{homicides\_df}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{homicides\_df }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}\StringTok{"data/philly\_homicides.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
We convert the \texttt{homicides\_df} data frame into an \texttt{sf} object with \texttt{st\_as\_sf()}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(sf)}

\NormalTok{homicides\_sf }\OtherTok{\textless{}{-}} \FunctionTok{st\_as\_sf}\NormalTok{(homicides\_df, }\AttributeTok{coords =} \FunctionTok{c}\NormalTok{(}\StringTok{"POINT\_X"}\NormalTok{, }\StringTok{"POINT\_Y"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{names}\NormalTok{(homicides\_sf)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{names}\NormalTok{(homicides\_df)}
\end{Highlighting}
\end{Shaded}
...
The \texttt{st\_geometry}function provides some helpful details about the spatial geometry of the object we created:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{st\_geometry}\NormalTok{(homicides\_sf)}
\end{Highlighting}
\end{Shaded}
...
We now use \texttt{st\_crs()} to check on the projection.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{st\_crs}\NormalTok{(homicides\_sf)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{st\_crs}\NormalTok{(homicides\_sf) }\OtherTok{\textless{}{-}} \DecValTok{4326}
\FunctionTok{st\_crs}\NormalTok{(homicides\_sf)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{st\_crs}\NormalTok{(homicides\_sf)}\SpecialCharTok{$}\NormalTok{proj4string}
\end{Highlighting}
\end{Shaded}
...
Here we assigned the projection in a separate step. You can also assign it at the same time when you read the csv file in with \texttt{st\_as\_sf}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{homicides\_sf }\OtherTok{\textless{}{-}} \FunctionTok{st\_as\_sf}\NormalTok{(homicides\_df, }\AttributeTok{coords =} \FunctionTok{c}\NormalTok{(}\StringTok{"POINT\_X"}\NormalTok{, }\StringTok{"POINT\_Y"}\NormalTok{), }
                         \AttributeTok{crs =} \FunctionTok{st\_crs}\NormalTok{(}\DecValTok{4326}\NormalTok{)) }\CommentTok{\# assign CRS here}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{st\_write}\NormalTok{(homicides\_sf, }\StringTok{"data/PhillyHomicides"}\NormalTok{, }\AttributeTok{driver =} \StringTok{"ESRI Shapefile"}\NormalTok{)}
\CommentTok{\# to force the save: }
\FunctionTok{st\_write}\NormalTok{(homicides\_sf, }\StringTok{"data/PhillyHomicides"}\NormalTok{, }\AttributeTok{driver =} \StringTok{"ESRI Shapefile"}\NormalTok{, }\AttributeTok{delete\_layer =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Read in Schools.csv, convert to a spatial sf object and save out as a shapefile. Assume that the coordinates are in WGS84 geographic projection.
I should mention that sf works with piping. You may or may not find that convenient.
\end{quote}

\section{Loading shape files into R}\label{loading-shape-files-into-r}

\texttt{sf} relies on the powerful \href{http://gdal.org}{GDAL library}, which is automatically linked in when loading \texttt{sf}. The GDAL provides the functionality to read and write spatial files of many formats. The command \texttt{st\_drivers()} returns the available drivers.

For shape files we can use \texttt{st\_read()}, which takes the path of the directory with the shapefile as argument.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{str}\NormalTok{(philly\_sf) }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{st\_geometry}\NormalTok{(philly\_sf) }
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/plot-shp-sf-1.pdf}}

To make it a little cleaner, we can only plot the polygon boundariesn and forget about the attributes. For this we need to directly use the geometry column. We use the \texttt{st\_geometry()} function again here, which extracts the \texttt{sfc} object (the simple feature geometry list column):
...
\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/plot-shp-sfg-1.pdf}}

Alternatively, we can choose the attributes that we what to plot:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# plot {-} alternatives}
\FunctionTok{plot}\NormalTok{(philly\_sf[}\StringTok{"medHHinc"}\NormalTok{]) }\CommentTok{\# subset with base R}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/plot-shp-attr-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{philly\_sf }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{select}\NormalTok{(medHHinc) }\SpecialCharTok{\%\textgreater{}\%}  \FunctionTok{plot}\NormalTok{() }\CommentTok{\# piping with tidyverse}
\end{Highlighting}
\end{Shaded}

Now let's get a little fancier.

Let's add a subset of polygons with only the census tracts where the median houshold income (\emph{medHHinc}) is more than \$60,000. First we plot the polygon. Then in a second step we extract elements from an \texttt{sf} object based on attributes using the dplyr filter function (base R subsetting also works) and add the census tracts to the plot in a different color.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# step 1 {-} plot empty polylgons:}
\FunctionTok{plot}\NormalTok{(}\FunctionTok{st\_geometry}\NormalTok{(philly\_sf)) }

\CommentTok{\# step 2 {-} add colored polygons on top}
...
\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/pipe-subset-plot-shp-sfg-1.pdf}}
...
#> source(s)   : memory
#> name        :        lyr.1 
#> min value   : 0.0005268205 
#> max value   : 0.9975202840
\end{verbatim}
...
\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/unnamed-chunk-9-1.pdf}}
...
\item
  Where on the system are the cell values stored?
\item
  What is the CRS?
\end{enumerate}
\end{quote}
...
\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/plot-raster-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/plot-raster-single-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/one-multiband-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/n-hist-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/n-hist-allvals-1.pdf}}
...
There are a wide variety of spatial, topological, and attribute data operations you can perform with R. \href{https://r.geocompx.org/}{Lovelace et al's recent publication}\footnote{Lovelace, R., Nowosad, J., \& Muenchow, J. (2025). Geocomputation with R. CRC Press.} goes into great depth about this and is highly recommended.
...
First we will load the CSV table \texttt{PhiladelphiaEduAttain.csv} into a dataframe in R and name it \texttt{edu}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{edu }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}\StringTok{"data/PhiladelphiaEduAttain.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{names}\NormalTok{(edu)}
\end{Highlighting}
\end{Shaded}

If you don't have the object still loaded read the the \texttt{Philly} shapefile into an object named \texttt{philly\_sf}. Check out the column names of \texttt{philly\_sf} and of \texttt{edu} to determine which one might contain the unique identifier for the join.
...
To join the \texttt{edu} data frame with \texttt{philly\_sf} we can use \texttt{join} like this:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{edu\_sf }\OtherTok{\textless{}{-}} \FunctionTok{left\_join}\NormalTok{(philly\_sf, edu, }\AttributeTok{by =} \FunctionTok{c}\NormalTok{(}\StringTok{"GEOID10"} \OtherTok{=} \StringTok{"GEOID"}\NormalTok{))}
\FunctionTok{names}\NormalTok{(edu\_sf) }
\end{Highlighting}
\end{Shaded}
...
1.Given what you have learned so far about plotting attributes, use the attributes
in edu\_sf, add a new variable and calculate the ratio of the female population with a high school education out of the total population. (Hint: you can use mutate)
...
\tightlist
\item
  Given what you have learned so far about plotting attributes create a simple
  plot that displays the ratio you just calculated.
\end{enumerate}
\end{quote}
...
These coordinates are already in the \emph{USA Contiguous Albers Equal Area Conic} projected CRS, which is the same as CRS as \texttt{philly\_sf}. We can use \texttt{str\_crs} to extract the projection definition from \texttt{philly\_sf}, so we don't have to type it out and then directly assign it to our new centroid object.

With this information, we create an object that holds the coordinates of the city center. Since we don't have attributes we will only create it as a simple feature collection, \texttt{scf} (not \texttt{sf}).
...
\NormalTok{philly\_ctr }\OtherTok{\textless{}{-}} 
  ...
  \FunctionTok{st\_sfc}\NormalTok{(}\AttributeTok{crs =} \FunctionTok{st\_crs}\NormalTok{(philly\_sf))  }\CommentTok{\# create feature collection, setting CRS to philly\_sf\textquotesingle{}s CRS}
\end{Highlighting}
\end{Shaded}

For the spatial operations we can take advantage of the suite of geometric operations that come with the \texttt{sf} package.

Let's check again the metric units of our CRS:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{st\_crs}\NormalTok{(philly\_ctr)}\SpecialCharTok{$}\NormalTok{proj4string}
\end{Highlighting}
\end{Shaded}
...
We use the buffer to select all census tract polygons that intersect with the center buffer. For this step we take advantage of another set of functions in \texttt{sf} that allow us to select by location. We will use \texttt{st\_filter} which ``keeps all x that overlap with y.''
...
\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/sf-intersects-plot-1.pdf}}

Note the difference to \texttt{st\_intersection}, which performs an actual clipping operation and creates an \texttt{sfc} object that cuts out the area of the buffer from the census polygons:
...
\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/sf-intersection-1.pdf}}
...
You can reproject a \texttt{sf} object with `st\_transform().
...
Let us go back to the \texttt{"PhillyHomicides"} shapefile we exported earlier. Let's reproject it so it matches the projection of the Philadelphia Census tracts.
...
\CommentTok{\#homicides\_sf \textless{}{-} st\_read("data/PhillyHomicides/")}
...
We use \texttt{st\_transform} and assign the result to a new object. Note how we can again use \texttt{str\_crs} to extract the projection definition from \texttt{philly\_sf}, so we don't have to type it out.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{homicides\_sf\_aea }\OtherTok{\textless{}{-}} \FunctionTok{st\_transform}\NormalTok{(homicides\_sf, }\FunctionTok{st\_crs}\NormalTok{(philly\_sf))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{range}\NormalTok{(}\FunctionTok{st\_coordinates}\NormalTok{(homicides\_sf))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{range}\NormalTok{(}\FunctionTok{st\_coordinates}\NormalTok{(homicides\_sf\_aea))}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{plot}\NormalTok{(}\FunctionTok{st\_geometry}\NormalTok{(homicides\_sf), }\AttributeTok{pch =} \DecValTok{20}\NormalTok{, }\AttributeTok{col=}\StringTok{"\#ccc"}\NormalTok{, }
     \AttributeTok{axes=}\NormalTok{F, }\AttributeTok{main =} \StringTok{"before transform {-} WGS84"}\NormalTok{)}
\FunctionTok{axis}\NormalTok{(}\AttributeTok{side =} \DecValTok{1}\NormalTok{, }\AttributeTok{las =} \DecValTok{3}\NormalTok{) }\CommentTok{\# adjust text on axes}
\FunctionTok{axis}\NormalTok{(}\AttributeTok{side =} \DecValTok{2}\NormalTok{, }\AttributeTok{las =} \DecValTok{1}\NormalTok{)}
\FunctionTok{plot}\NormalTok{(}\FunctionTok{st\_geometry}\NormalTok{(homicides\_sf\_aea), }\AttributeTok{pch =} \DecValTok{20}\NormalTok{, }\AttributeTok{col=}\StringTok{"\#ccc"}\NormalTok{, }
     \AttributeTok{axes=}\NormalTok{F, }\AttributeTok{main =} \StringTok{"after transform {-} AEA"}\NormalTok{)}
\FunctionTok{axis}\NormalTok{(}\AttributeTok{side =} \DecValTok{1}\NormalTok{, }\AttributeTok{las =} \DecValTok{3}\NormalTok{)}
\FunctionTok{axis}\NormalTok{(}\AttributeTok{side =} \DecValTok{2}\NormalTok{, }\AttributeTok{las =} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/compare-reproj-plots-sf-1.pdf}}

Lastly, let us save the reprojected file as \texttt{PhillyHomicides\_aea} shapefile, as we will use it later on.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{st\_write}\NormalTok{(homicides\_sf\_aea, }\StringTok{"data/PhillyHomicides\_aea"}\NormalTok{, }\AttributeTok{driver =} \StringTok{"ESRI Shapefile"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/raster-reproject-plot1-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/raster-reproject-plot2-1.pdf}}

\section{Spatial Aggregation: Points in Polygon}\label{spatial-aggregation-points-in-polygon}
...
\begin{Highlighting}[]
\NormalTok{philly\_sf }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{tract\_area =} \FunctionTok{st\_area}\NormalTok{(geometry)) }
\end{Highlighting}
\end{Shaded}
...
\NormalTok{philly\_sf }\SpecialCharTok{\%\textgreater{}\%} 
  ...
  \FunctionTok{st\_join}\NormalTok{(homicides\_sf\_aea) }
\end{Highlighting}
\end{Shaded}

Now we can group by a variable that uiquely identifies the census tracts, (we choose \emph{GEOID10}) and use \texttt{summarize} to count the points for each tract and calculate the homicide rate. Since our units are in sq meter we multiply by by 1000000 to get sq km. We also need to carry over the area, which I do using \texttt{unique}. \texttt{unique} returns the object, but with duplicate elements/rows removed. \texttt{as.numeric} coerces the result to numeric instead of the value in units of {[}1/m2{]}.

We also assign the output to a new object \texttt{crimes\_sf}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{crimes\_sf }\OtherTok{\textless{}{-}}\NormalTok{ philly\_sf }\SpecialCharTok{\%\textgreater{}\%}
      \FunctionTok{mutate}\NormalTok{(}\AttributeTok{tract\_area =} \FunctionTok{st\_area}\NormalTok{(geometry)) }\SpecialCharTok{\%\textgreater{}\%}
      \FunctionTok{st\_join}\NormalTok{(homicides\_sf\_aea) }\SpecialCharTok{\%\textgreater{}\%}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{st\_write}\NormalTok{(crimes\_sf, }\StringTok{"data/PhillyCrimerate"}\NormalTok{, }\AttributeTok{driver =} \StringTok{"ESRI Shapefile"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/chm-1.pdf}}
...
The \texttt{terra} package contains a function called \texttt{lapp()}function to make processing more efficient. It takes two or more rasters and applies a function to them. The generic syntax is:
...
\texttt{ggplot} can plot \texttt{sf} objects directly by using the geom \texttt{geom\_sf}. So we can take advantage of this and say:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# if you need to read this in again:}
\CommentTok{\# crimes\_sf \textless{}{-} st\_read("data/PhillyCrimerate") }
\FunctionTok{ggplot}\NormalTok{(crimes\_sf) }\SpecialCharTok{+} 
  \FunctionTok{geom\_sf}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{fill=}\NormalTok{homic\_rate))}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/ggplot-sf-1.pdf}}

We can add more map layers to ggplot with the familiar ``+'' sign:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# read in another dataset}
\NormalTok{rooselvelt\_sf }\OtherTok{\textless{}{-}} \FunctionTok{st\_read}\NormalTok{(}\StringTok{"data/Roosevelt\_blvd/"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
#> Reading layer `Roosevelt_blvd' from data source 
#>   `/Users/cengel/Anthro/R_Class/R_Workshops/R-spatial/data/Roosevelt_blvd' 
#>   using driver `ESRI Shapefile'
#> Simple feature collection with 1 feature and 1 field
#> Geometry type: LINESTRING
#> Dimension:     XY
#> Bounding box:  xmin: 1746511 ymin: 472799.6 xmax: 1760254 ymax: 487615.1
#> Projected CRS: Albers
\end{verbatim}
...
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(crimes\_sf) }\SpecialCharTok{+} 
  \FunctionTok{geom\_sf}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{fill=}\NormalTok{homic\_rate)) }\SpecialCharTok{+}
  \FunctionTok{geom\_sf}\NormalTok{(}\AttributeTok{data =}\NormalTok{ rooselvelt\_sf, }\AttributeTok{color =} \StringTok{"yellow"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/ggplot-sf-addlayer-1.pdf}}

Homicide rate is a continuous variable and is indicated in legend by \texttt{ggplot} as such. To modify the legend to resemble more closely `choropleth' map legend we need to convert our continuous variable into a categorical one, according to whichever brackets we want to use.
...
\NormalTok{breaks\_qt }\OtherTok{\textless{}{-}} \FunctionTok{classIntervals}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\FunctionTok{min}\NormalTok{(crimes\_sf}\SpecialCharTok{$}\NormalTok{homic\_rate) }\SpecialCharTok{{-}}\NormalTok{ .}\DecValTok{00001}\NormalTok{,}
\NormalTok{                              crimes\_sf}\SpecialCharTok{$}\NormalTok{homic\_rate), }
...
#>  - attr(*, "nobs")= int 385
#>  - attr(*, "call")= language classIntervals(var = c(min(crimes_sf$homic_rate) - 1e-05, crimes_sf$homic_rate),      n = 7, style = "quantile")
...
We use \texttt{cut} to divide \texttt{homic\_rate} into intervals and code them according to which interval they are in.

Lastly, we can use \texttt{scale\_fill\_brewer} and add our color palette.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{crimes\_sf }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{homic\_rate\_cat =} \FunctionTok{cut}\NormalTok{(homic\_rate, breaks\_qt}\SpecialCharTok{$}\NormalTok{brks)) }\SpecialCharTok{\%\textgreater{}\%} 
...
\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/ggplot-sf-categorical-1.pdf}}
...
Before using ggplot we need to convert it to a dataframe. The \texttt{terra} package has an built-in function for conversion to a plot-able dataframe.
...
\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/ggplot-rast-1.pdf}}

\section{\texorpdfstring{Choropleth with \texttt{tmap}}{Choropleth with tmap}}\label{choropleth-with-tmap}

\texttt{tmap} is specifically designed to make creation of thematic maps more convenient. It borrows from the ggplot syntax and takes care of a lot of the styling and aesthetics. This reduces our amount of code significantly. We need:
...
\FunctionTok{tm\_shape}\NormalTok{(crimes\_sf) }\SpecialCharTok{+}
  \FunctionTok{tm\_polygons}\NormalTok{(}\AttributeTok{fill =} \StringTok{"homic\_rate"}\NormalTok{,}
              \AttributeTok{fill.scale =} \FunctionTok{tm\_scale\_intervals}\NormalTok{(}\AttributeTok{style=}\StringTok{"quantile"}\NormalTok{),}
              \AttributeTok{fill.legend =} \FunctionTok{tm\_legend}\NormalTok{(}\AttributeTok{title =} \StringTok{"Philadelphia }\SpecialCharTok{\textbackslash{}n}\StringTok{homicides }\SpecialCharTok{\textbackslash{}n}\StringTok{per sqKm}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/tmap-plot-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{R-spatial_files/figure-latex/unnamed-chunk-15-1.pdf}}
...
And while \texttt{tmap} was tolerant about our AEA projection of \texttt{crimes\_sf}, \texttt{leaflet} does require us to explicitly reproject the \texttt{sf} object.
...
\NormalTok{philly\_WGS84 }\OtherTok{\textless{}{-}} \FunctionTok{st\_transform}\NormalTok{(crimes\_sf, }\DecValTok{4326}\NormalTok{)}
