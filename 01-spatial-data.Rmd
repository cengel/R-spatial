...
```{r spsf-dl, echo=FALSE, results='asis', out.width='100%', fig.cap='Cumulative downloads of sf and sp from CRAN'}
...
```{r names-sf, results='show'}
names(homicides_sf)
```

Compare this with the original table:
```{r names-df, results='show'}
names(homicides_df)
```
...
The `st_geometry`function provides some helpful details about the spatial geometry of the object we created:

```{r st-geom, results='show'}
st_geometry(homicides_sf)
```

And here is what we see when we type the name of the sf object:
```{r st-obj, results='show'}
homicides_sf
```
...
st_read("data/Schools.csv") |> 
  sr_as_sf(coords = c("X", "Y", crs = st_crs(4326))) |> 
  st_write("data/PhillySchools", driver = "ESRI Shapefile")
...
`sf` relies on the powerful [GDAL library](http://gdal.org), which is automatically linked in when loading `sf`. The GDAL provides the  functionality to read and write spatial files of many formats.  The command `st_drivers()` returns the available drivers.
...
```{r str-shp-sf, eval=TRUE, results='show'}
str(philly_sf) 
```
...
```{r geo-shp-sf, eval=TRUE, results='show'}
...
```{r plot-shp-attr, eval=TRUE, results='show'}
# plot - alternatives
